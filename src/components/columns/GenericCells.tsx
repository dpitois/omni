...
export function DateCell({ value, onUpdate, onKeyDown, onFocus }: DateCellProps) {
  return (
    <div className="relative w-full flex items-center group/date">
      <Calendar
        size={12}
        className={`absolute left-0 transition-colors ${value ? 'text-blue-500' : 'text-text-dim/30 group-hover/date:text-text-dim'}`}
      />
      <input
        type="date"
        value={value}
...
export function TextCell({ value, onUpdate, onKeyDown, onFocus }: TextCellProps) {
  return (
    <div className="relative w-full flex items-center group/textcol">
      <Type
        size={12}
        className="absolute left-0 text-text-dim/20 group-hover/textcol:text-text-dim/40 transition-colors"
      />
      <input
        type="text"
        placeholder="..."
        ...
        onKeyDown={onKeyDown}
        onFocus={onFocus}
        className="w-full bg-transparent border-none outline-none text-[11px] pl-5 text-text-main placeholder:text-text-dim/20 cursor-pointer"
      />
    </div>
  );
}
