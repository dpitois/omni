import { ChevronRight, Check, Minus } from 'lucide-preact';
      ...
      <div className="w-8 h-8 flex items-center justify-center relative">
        {hasChildren ? (
          <button
            ...
            className={`p-0.5 rounded hover:bg-item-hover text-text-dim hover:text-text-main transition-all duration-300 transform ${collapsed ? '' : 'rotate-90'}`}
          >
        ...
        {collapsed && hasChildren && (
          <div className="absolute -right-1 top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-500/20 text-blue-500 text-[10px] flex items-center justify-center rounded-full font-bold animate-in zoom-in duration-300">
            +
          </div>
        )}
      </div>
